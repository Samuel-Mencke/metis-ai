...
export async function POST(req: Request) {
  ...
  const ownerId = await getAuthenticatedUserId(req);
  if (!ownerId) return Response.json({ error: "Unauthorized" }, { status: 401 });
  const requestBody = (await req.json().catch(() => ({}))) as { os?: unknown };
  ...
  const command = `curl -fsSL ${publicUrl}/install/remote-client.sh | bash -s -- --server ${publicUrl} --enrollment-token ${token.token}`;
  const windowsCommand = `& ([scriptblock]::Create((irm ${publicUrl}/install/remote-client.ps1))) -Server '${publicUrl}' -EnrollmentToken '${token.token}'`;
  const macosCommand = `curl -fsSL ${publicUrl}/install/remote-client-macos.sh | bash -s -- --server ${publicUrl} --enrollment-token ${token.token}`;
