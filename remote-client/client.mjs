...
async function execute(action, params = {}) {
  ...
  if (action === "read_file") { const file = String(params.path || ""); if (!file) throw new Error("Path is required"); const limit = Math.min(Number(params.limit) || 5_000_000, 10_000_000); return { path: file, content: fs.readFileSync(file, "utf8").slice(0, limit) }; }
  ...
  if (action === "upload_file") {
    ...
    const response = await fetch(`${server}/api/internal/control-artifacts`, {
      method: "POST",
      headers: {
        authorization: `Remote ${config.clientId}:${config.credential}`,
        "content-type": mimeType,
        "x-control-run-id": runId,
