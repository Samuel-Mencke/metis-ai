...
const MAX_TEXT_RESPONSE = 48_000;
const clipTail = (value, max) => {
  const text = String(value || "");
  if (Buffer.byteLength(text, "utf8") <= max) return text;
  const data = Buffer.from(text, "utf8");
  return `[... ${data.length - max} bytes truncated; showing tail ...]\n${data.subarray(data.length - max).toString("utf8")}`;
};
...
async function execute(action, params = {}) {
  if (action === "get_info") return { hostname: os.hostname(), os: `${process.platform} ${os.release()}`, architecture: process.arch, version: "1.3.0", cwd: process.cwd(), memory: { total: os.totalmem(), free: os.freemem() }, uptime: os.uptime() };
  if (action === "execute_command") {
    ...
    const result = await execFileAsync(shell, process.platform === "win32" ? ["/d", "/s", "/c", command] : ["-lc", command], { cwd, timeout, maxBuffer: 32 * 1024 * 1024, windowsHide: true });
    return { stdout: clipTail(result.stdout, 39_000), stderr: clipTail(result.stderr, 7_000), exitCode: 0, outputTruncated: Buffer.byteLength(String(result.stdout || ""), "utf8") > 39_000 || Buffer.byteLength(String(result.stderr || ""), "utf8") > 7_000 };
  }
  if (action === "list_directory") {
    const directory = String(params.path || os.homedir());
    const offset = Math.max(0, Number(params.offset) || 0);
    const limit = Math.max(1, Math.min(Number(params.limit) || 500, 1000));
    const all = fs.readdirSync(directory, { withFileTypes: true });
    return { path: directory, offset, total: all.length, nextOffset: offset + limit < all.length ? offset + limit : null, entries: all.slice(offset, offset + limit).map((entry) => ({ name: entry.name, directory: entry.isDirectory() })) };
  }
  if (action === "read_file") {
    const file = String(params.path || ""); if (!file) throw new Error("Path is required");
    const data = fs.readFileSync(file);
    const offset = Math.max(0, Number(params.offset) || 0);
    const limit = Math.max(1, Math.min(Number(params.limit) || MAX_TEXT_RESPONSE, MAX_TEXT_RESPONSE));
    const end = Math.min(data.length, offset + limit);
    return { path: file, offset, totalBytes: data.length, nextOffset: end < data.length ? end : null, content: data.subarray(offset, end).toString("utf8") };
  }
  ...
  if (action === "pty_open") { const child = spawn(shell, process.platform === "win32" ? [] : ["-i"], { cwd: typeof params.cwd === "string" ? params.cwd : os.homedir(), env: process.env, stdio: "pipe", windowsHide: true }); const sessionId = crypto.randomUUID(); running.set(sessionId, child); child.stdout.on("data", (data) => send({ type: "event", sessionId, event: "stdout", data: clipTail(data.toString(), MAX_TEXT_RESPONSE) })); child.stderr.on("data", (data) => send({ type: "event", sessionId, event: "stderr", data: clipTail(data.toString(), MAX_TEXT_RESPONSE) })); child.on("exit", (code) => { running.delete(sessionId); send({ type: "event", sessionId, event: "exit", code }); }); return { sessionId }; }
  ...
  if (action === "pty_close") { const child = running.get(String(params.sessionId)); child?.kill(); return { ok: true }; }
