...
import { Client } from "/home/f1shy312/ai-chat/node_modules/.pnpm/node_modules/@modelcontextprotocol/sdk/dist/esm/client/index.js";
import { StdioClientTransport } from "/home/f1shy312/ai-chat/node_modules/.pnpm/node_modules/@modelcontextprotocol/sdk/dist/esm/client/stdio.js";
import { StreamableHTTPClientTransport } from "/home/f1shy312/ai-chat/node_modules/.pnpm/node_modules/@modelcontextprotocol/sdk/dist/esm/client/streamableHttp.js";
import { Server } from "/home/f1shy312/ai-chat/node_modules/.pnpm/node_modules/@modelcontextprotocol/sdk/dist/esm/server/index.js";
import { StdioServerTransport } from "/home/f1shy312/ai-chat/node_modules/.pnpm/node_modules/@modelcontextprotocol/sdk/dist/esm/server/stdio.js";
import { StreamableHTTPServerTransport } from "/home/f1shy312/ai-chat/node_modules/.pnpm/node_modules/@modelcontextprotocol/sdk/dist/esm/server/streamableHttp.js";
import {
  isInitializeRequest,
  ListToolsRequestSchema,
  CallToolRequestSchema,
} from "/home/f1shy312/ai-chat/node_modules/.pnpm/node_modules/@modelcontextprotocol/sdk/dist/esm/types.js";

const PORT = Number.parseInt(process.env.MCP_PORT || "8787", 10);
const ROOT = process.env.AI_CHAT_MCP_ROOT || process.cwd();
const STATE_DIR = process.env.AI_CHAT_MCP_STATE_DIR || path.join(process.cwd(), "data", "mcp-state");
...
const DEFAULT_REGISTRY = [
  ...
  },
  {
    ...
    kind: "stdio",
    command: "npx",
    args: ["-y", "@modelcontextprotocol/server-filesystem@latest", os.homedir()],
    enabled: true,
    tags: ["files", "server"],
  },
  {
...
async function runShell({ target = "server", command, timeout = 60, cwd, sudo = false }) {
  ...
  const workingDir = cwd || t.home;
  const inner = `cd ${q(workingDir)} && ${command}`;
  const finalCmd = sudo ? `sudo -n bash -lc ${q(inner)}` : inner;
...
async function runWindowsJob(job, waitSeconds = undefined) {
  ...
  const base = `${pcHome}\\.samuel-mcp-runner`;
...
async function getChild(serverId) {
  const registry = await loadRegistry();
...
async function childTools(serverId, force = false) {
  const child = await getChild(serverId);
...
async function callChild(serverId, toolName, args = {}) {
  const child = await getChild(serverId);
...
const targetProp = {
  type: "string",
  enum: Object.keys(HOSTS),
  default: "server",
  description: `Available targets: ${Object.keys(HOSTS).join(", ")}.`,
};

const tools = [
  {
    name: "gateway_status",
    description: "Show gateway health, devices, enabled child MCP servers, and Windows desktop runner state.",
  ...
  {
    name: "ask_user",
    description: "Pause the agent and ask the signed-in AI Chat user one or more questions. The call remains pending until the user answers in the chat UI.",
    inputSchema: {
      type: "object",
      properties: {
        questions: {
  ...
  {
    name: "read_file",
    description: "Read a UTF-8 text file from any device.",
  ...
  {
    name: "write_file",
    description: "Create or overwrite a UTF-8 text file on any device.",
  ...
  {
    name: "windows_runner_status",
    description: "Check whether the interactive Windows desktop automation runner is active and able to see real windows.",
    inputSchema: { type: "object", properties: {}, additionalProperties: false },
    annotations: { readOnlyHint: true },
  },
  {
...
async function dispatchGatewayTool(name, args = {}, { auditCall = true, context = {} } = {}) {
    if (auditCall) await audit(name, args);
    try {
      if (name === "gateway_status") {
        const registry = await loadRegistry();
        const winRunnerPath = HOSTS.pc ? `${HOSTS.pc.home}\\.samuel-mcp-runner\\status.json` : null;
        const win = winRunnerPath ? await runWindowsPowerShell(`Get-Content -Raw -LiteralPath '${winRunnerPath}' -ErrorAction SilentlyContinue`, 15) : { stdout: "", stderr: "", exit_code: -1 };
        let runner = null;
        try { runner = JSON.parse(win.stdout.trim()); } catch { runner = { reachable: false, raw: win.stdout, error: win.stderr }; }
        return asText({
          name: "Samuel Universal MCP Gateway",
          version: "4.0.0",
      ...
      if (name === "search_tools") {
        const registry = (await loadRegistry()).filter((s) => s.enabled && (!args.server || s.id === args.server));
        ...
        for (const entry of registry) {
          try {
            const serverTools = await childTools(entry.id);
      ...
      if (name === "list_server_tools") return asText(await childTools(args.server, Boolean(args.refresh)));
      if (name === "call_mcp_tool") return returnChildResult(await callChild(args.server, args.tool, args.arguments || {}));
      if (name === "ask_user") {
        const chatId = String(context.chatId || "").trim();
        const jobId = String(context.jobId || "").trim();
        if (!chatId || !jobId) throw new Error("ask_user requires an AI Chat session context");
        const baseUrl = process.env.AI_CHAT_INTERNAL_URL?.trim() || "http://127.0.0.1:3100/api/internal/mcp-question";
        ...
        if (!response.ok) throw new Error(`AI Chat question endpoint failed (HTTP ${response.status}): ${body.slice(0, 500)}`);
      ...
      if (name === "ensure_capability") {
        ...
        for (const entry of (await loadRegistry()).filter((server) => server.enabled && server.id !== "registry-autobroker")) {
          try {
            for (const tool of await childTools(entry.id)) {
      ...
      if (name === "upsert_mcp_server") {
        ...
        const registry = await loadRegistry();
        const existing = registry.findIndex((s) => s.id === args.id);
        const entry = { ...(existing >= 0 ? registry[existing] : {}), ...args };
      ...
      if (name === "set_mcp_server_enabled") {
        const registry = await loadRegistry();
        const entry = registry.find((s) => s.id === args.server);
      ...
      if (name === "windows_runner_status") {
        const runnerStatusPath = HOSTS.pc ? `${HOSTS.pc.home}\\.samuel-mcp-runner\\status.json` : null;
        const result = runnerStatusPath ? await runWindowsPowerShell(`Get-Content -Raw -LiteralPath '${runnerStatusPath}'; Get-ScheduledTaskInfo -TaskName 'Samuel MCP Desktop Runner' | Select-Object LastRunTime,LastTaskResult | ConvertTo-Json`, 30) : { stdout: JSON.stringify({ reachable: false, error: "No PC device configured" }), stderr: "", exit_code: -1 };
      ...
      if (name === "workflow_run") {
        ...
        return asText(await runWorkflow(workflow, {
          ...
          execute: async (tool, toolArgs) => {
            const result = await dispatchGatewayTool(tool, toolArgs);
            const text = childResultText(result);
      ...
      if (name === "get_connection_instructions") {
        ...
        const allClients = {
          ...
          "codex": { name: "OpenAI Codex CLI", config: { mcpServers: { "samuel-universal": { type: "streamable-http", url: `${PUBLIC_URL}/all`, headers: { Authorization: `Bearer ${token}` } } } } },
          "hermes": { name: "Hermes Agent", config: { type: "streamable-http", url: `${PUBLIC_URL}/all`, headers: { Authorization: `Bearer ${token}` } } },
          "gemini": { name: "Gemini CLI", config: { mcpServers: { "samuel-universal": { url: `${PUBLIC_URL}/all`, transport: "streamable-http", headers: { Authorization: `Bearer ${token}` } } } } },
          "opencode": { name: "OpenCode", config: { mcp: { "samuel-universal": { type: "streamable-http", url: `${PUBLIC_URL}/all`, headers: { Authorization: `Bearer ${token}` } } } } },
          "cursor": { name: "Cursor", config: { mcpServers: { "samuel-universal": { url: `${PUBLIC_URL}/all`, transport: "streamable-http", headers: { Authorization: `Bearer ${token}` } } } } },
        };
        if (args.client) {
...
async function main() {
  ...
  const httpServer = http.createServer(async (req, res) => {
    ...
    if (url.pathname === "/health") {
      ...
      res.end(JSON.stringify({ ok: true, name: "Samuel Universal MCP Gateway", version: "4.0.0", endpoint: "/mcp" }));
      return;
    }
    if (!authorized(req)) {
    ...
    if (url.pathname !== "/mcp" && url.pathname !== "/") { res.writeHead(404); res.end("Not Found"); return; }
  ...
  httpServer.listen(PORT, "127.0.0.1", () => console.log(`[mcp] Samuel Universal MCP Gateway listening on http://127.0.0.1:${PORT}/mcp`));
