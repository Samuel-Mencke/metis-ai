...
function assertToolPolicy(name, context = {}) {
  if (
...
const tools = [
  ...
  {
    name: "ask_user",
    description: `Pause the agent and ask the signed-in ${APP_NAME} user one or more questions. The call remains pending until the user answers in the chat UI.`,
    inputSchema: {
      type: "object",
      properties: {
        questions: {
...
async function dispatchGatewayTool(name, args = {}, { auditCall = true, context = {} } = {}) {
    ...
    try {
      ...
      if (name === "create_plan" || name === "create_canvas") {
        const type = name === "create_plan" ? "plan" : "canvas";
      ...
      if (name === "gateway_status") {
        const registry = visibleRegistry(await loadRegistry(), context);
        const winRunnerPath = HOSTS.pc ? `${HOSTS.pc.home}\\.metis-ai-mcp-runner\\status.json` : null;
      ...
      if (name === "call_mcp_tool") return returnChildResult(await callChild(args.server, args.tool, args.arguments || {}, context));
      if (name === "ask_user") {
        const chatId = String(context.chatId || "").trim();
        const jobId = String(context.jobId || "").trim();
      ...
      if (["browser_navigate", "browser_snapshot", "browser_screenshot", "browser_click", "browser_type", "browser_press", "browser_scroll", "browser_resize", "browser_tabs"].includes(name)) {
        const chatId = String(context.chatId || "").trim();
        const userId = String(context.userId || "").trim();
        if (!chatId || !userId) throw new Error(`${name} requires a chat session context`);
        const actionMap = {
          browser_navigate: "navigate", browser_snapshot: "snapshot", browser_screenshot: "screenshot",
        ...
        const payload = { action, tabId: args.tab_id, url: args.url, selector: args.selector, text: args.text, key: args.key, x: args.x, y: args.y, deltaY: args.delta_y, width: args.width, height: args.height };
      ...
      if (name === "windows_runner_status") {
        const runnerStatusPath = HOSTS.pc ? `${HOSTS.pc.home}\\.metis-ai-mcp-runner\\status.json` : null;
        const result = runnerStatusPath ? await runWindowsPowerShell(`Get-Content -Raw -LiteralPath '${runnerStatusPath}'; Get-ScheduledTaskInfo -TaskName '${APP_NAME} MCP Desktop Runner' | Select-Object LastRunTime,LastTaskResult | ConvertTo-Json`, 30) : { stdout: JSON.stringify({ reachable: false, error: "No PC device configured" }), stderr: "", exit_code: -1 };
