...
function visibleRegistry(registry, context = {}) {
  return context.userId ? registry.filter((entry) => entry.ownerId === context.userId) : registry;
}
...
async function dispatchGatewayTool(name, args = {}, { auditCall = true, context = {} } = {}) {
    ...
    try {
      ...
      if (name === "upsert_mcp_server") {
        ...
        const registry = await loadRegistry();
        const existing = registry.findIndex((s) => s.id === args.id && (!context.userId || s.ownerId === context.userId));
      ...
      if (name === "set_mcp_server_enabled") {
        const registry = await loadRegistry();
        const entry = registry.find((s) => s.id === args.server && (!context.userId || s.ownerId === context.userId));
