...
function modeToolCategory(name) {
  ...
  if (value === "gateway_status" || value === "search_tools" || value === "list_server_tools" || value === "list_mcp_servers" || value === "test_mcp_server" || value === "call_mcp_tool") return "read";
  if (value === "write_todos") return "plan";
  if (/^(list_notes|search_notes|create_note|update_note)$/.test(value)) return "plan";
...
async function assertSharedBrowserCommand(args = {}, context = {}) {
  ...
  for (const match of scriptMatches.slice(0, 4)) {
    try {
      ...
      const resolved = path.isAbsolute(raw) ? raw : path.resolve(String(args.cwd || process.env.MCP_AGENT_CWD || ROOT), raw);
...
async function dispatchGatewayTool(name, args = {}, { auditCall = true, context = {} } = {}) {
    ...
    try {
      ...
      if (name === "call_mcp_tool") {
        ...
        // The wrapper is only a transport. Enforce the selected child tool's
        // actual mode category before dispatching it.
        assertToolPolicy(childToolName, context, args.arguments || {});
      ...
      if (name === "repo_search" || name === "inspect_codebase" || name === "find_symbol") {
        const root = path.resolve(String(args.root || process.env.MCP_AGENT_CWD || ROOT));
        if (name === "find_symbol") {
      ...
      if (["browser_navigate", "browser_snapshot", "browser_screenshot", "browser_click", "browser_type", "browser_press", "browser_scroll", "browser_resize", "browser_tabs", "browser_extract_text", "browser_form_state", "browser_batch", "browser_wait_for", "browser_fill_form", "browser_download", "browser_drag", "browser_hover", "browser_select_option", "browser_upload_file"].includes(name)) {
        ...
        const payload = {
          ...
          width: args.width,
          height: args.height,
          downloadPath: process.env.MCP_AGENT_CWD || ROOT,
        };
    ...
    try {
      ...
      if (!sessionId && req.method === "POST") {
        ...
        const sessionServer = createServerForSession({
          transport: "http",
          // HTTP MCP clients are authenticated as gateway clients, not as
          // Metis accounts. Never trust caller-supplied user/chat/job headers
          // for user-scoped operations; the in-process stdio gateway carries
          // trusted context separately.
        });
        let transport;
