...
const tools = [
  {
    name: "provide_file",
    description: "Make a file from the agent workspace available to the user as a protected download link and attachment preview.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description: "Ask the user to confirm a potentially destructive or external action before proceeding.",
    inputSchema: {
      type: "object",
      properties: {
        action: { type: "string" },
...
async function dispatchGatewayTool(name, args = {}, { auditCall = true, context = {} } = {}) {
    ...
    try {
      ...
      if (name === "update_chat_keywords" || name === "search_chats") {
        const chatId = String(context.chatId || "").trim();
        const jobId = String(context.jobId || "").trim();
        if (!chatId || !jobId) throw new Error(`${name} requires a chat session context`);
      ...
      if (name === "request_confirmation") {
        const question = `Confirm ${String(args.action || "this action")}?${args.details ? `\n\n${String(args.details)}` : ""}`;
        return await dispatchGatewayTool("ask_user", {
          questions: [{ question, multiple: false, options: ["Confirm", "Cancel"] }],
        }, { context, auditCall: false });
      }
