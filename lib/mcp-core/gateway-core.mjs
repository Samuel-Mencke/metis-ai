...
async function connectChild(entry, serverId) {
...
async function getChild(serverId, context = {}) {
  ...
  const inflight = connectingChildren.get(serverId);
  if (inflight) return inflight;
  const pending = connectChild(entry, serverId).finally(() => connectingChildren.delete(serverId));
  connectingChildren.set(serverId, pending);
  return pending;
}
...
const tools = [
  ...
  },
  {
    ...
    description: "Create a one-time or recurring automation that runs an agent prompt in the target chat. Recurring schedules can use minutes, days, or a calendar day each month.",
    inputSchema: {
      type: "object",
      properties: {
        name: { type: "string" },
  ...
  {
    name: "write_todos",
    description: "Create or replace a short in-chat checklist. This is the default progress tracker; it appears in the message, not as a side-panel plan. Use create_plan only when the user asked for a plan document.",
    inputSchema: {
      type: "object",
      properties: {
        todos: {
  ...
  },
  {
    ...
    inputSchema: {
      type: "object",
      properties: { selector: { type: "string" }, target_selector: { type: "string" }, tab_id: { type: "string" } },
  ...
  },
  {
    ...
    inputSchema: {
      type: "object",
      properties: { selector: { type: "string" }, value: { type: "string" }, tab_id: { type: "string" } },
  ...
  },
  {
    ...
    inputSchema: {
      type: "object",
      properties: { selector: { type: "string" }, file_path: { type: "string" }, tab_id: { type: "string" } },
...
async function dispatchGatewayTool(name, args = {}, { auditCall = true, context = {} } = {}) {
    ...
    try {
      ...
      if (["browser_navigate", "browser_snapshot", "browser_screenshot", "browser_click", "browser_type", "browser_press", "browser_scroll", "browser_resize", "browser_tabs", "browser_extract_text", "browser_fill_form", "browser_download", "browser_drag", "browser_hover", "browser_select_option", "browser_upload_file"].includes(name)) {
        const chatId = String(context.chatId || "").trim();
        ...
        const payload = { action, tabId: args.tab_id, url: args.url, selector: args.selector, text: args.text, key: args.key, value: args.value, targetSelector: args.target_selector, filePath: args.file_path, x: args.x, y: args.y, deltaY: args.delta_y, width: args.width, height: args.height, downloadPath: process.env.MCP_AGENT_CWD || ROOT };
