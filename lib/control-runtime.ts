...
import {
  ...
  recoverInterruptedControlRuns,
  updateControlRun,
  type ControlRun,
} from "@/lib/control-plane";

const inFlight = new Set<string>();
const runtimeGlobal = globalThis as typeof globalThis & { __metisControlRuntimeStarted?: boolean };
...
async function collectArtifacts(run: ControlRun, manifestPath: string) {
  ...
  for (const item of manifest.slice(0, 40)) {
    ...
    try {
      await requestRemoteClient({ ownerId: run.ownerId, clientId: run.clientId, action: "upload_file" as any, params: { path: remotePath, runId: run.id, name, mimeType }, source: "agent", approved: true, timeoutMs: 180_000 });
    } catch (error) {
...
async function runIteration(run: ControlRun) {
  ...
  await requestRemoteClient({ ownerId: latest.ownerId, clientId: latest.clientId, action: "write_file", params: { path: promptPath, content: systemPrompt(latest, turnPrompt, manifestPath) }, source: "agent", approved: true, timeoutMs: 30_000 });
  const raw = await requestRemoteClient({ ownerId: latest.ownerId, clientId: latest.clientId, action: "execute_command", params: { command: buildAgyCommand({ promptPath, conversationId: latest.conversationId, model: latest.model, effort: latest.effort }), cwd: latest.cwd, timeout: 6 * 60 * 60_000 }, source: "agent", approved: true, timeoutMs: 6 * 60 * 60_000 + 60_000 }) as { stdout?: string; stderr?: string; exitCode?: number };
  ...
  if (getControlRun(latest.id, latest.ownerId)?.cancelRequested) {
  ...
  updateControlRun(latest.id, latest.ownerId, { state: "sleeping", iteration, resultText, unread: true, conversationId: parsed.conversationId || refreshed.conversationId || null, nextRunAt: new Date(Date.now() + refreshed.intervalSeconds * 1000).toISOString() });
}
...
async function tick() {
  for (const run of listRunnableControlRuns()) {
    ...
    void runIteration(run).catch((error) => {
      ...
      if (/offline|disconnect|timed out|ECONN|network/i.test(message)) updateControlRun(run.id, run.ownerId, { state: "sleeping", error: message, nextRunAt: new Date(Date.now() + 30_000).toISOString() });
...
export function startControlRuntime() {
  if (runtimeGlobal.__metisControlRuntimeStarted) return;
  runtimeGlobal.__metisControlRuntimeStarted = true;
  recoverInterruptedControlRuns();
