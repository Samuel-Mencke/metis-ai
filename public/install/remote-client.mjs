...
const wsUrl = String(config.server).replace(/\/+$/, "").replace(/^http:/, "ws:").replace(/^https:/, "wss:") + "/ws/remote-client";
const shell = process.platform === "win32" ? (process.env.ComSpec || "cmd.exe") : (process.env.SHELL || "/bin/sh");
const running = new Map();
async function execute(action, params = {}) {
  if (action === "get_info") return { hostname: os.hostname(), os: `${process.platform} ${os.release()}`, architecture: process.arch, version: "1.0.0", cwd: process.cwd(), memory: { total: os.totalmem(), free: os.freemem() }, uptime: os.uptime() };
  if (action === "execute_command") {
    const command = String(params.command || ""); if (!command.trim()) throw new Error("Command is required");
    const result = await execFileAsync(shell, process.platform === "win32" ? ["/d", "/s", "/c", command] : ["-lc", command], { cwd: params.cwd || os.homedir(), timeout: Math.max(1_000, Math.min(Number(params.timeout) || 60_000, 300_000)), maxBuffer: 2 * 1024 * 1024, windowsHide: true });
